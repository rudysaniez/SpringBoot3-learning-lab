...
public class DictionaryHealthMockServerLoader extends CommonBaseFunction {

    public static final String HEALTH_PATH = "/management/health";

    public static final Map<String, HttpRequest> REQUEST_MAP = new HashMap<>();

    private static final Logger log = LoggerFactory.getLogger(DictionaryHealthMockServerLoader.class);
    ...
    public static void load(@NotNull ClientAndServer attributeMockServer) throws Exception {
        ...
        REQUEST_MAP.put(HEALTH_PATH, request);

        attributeMockServer.when(request)
                .respond(HttpResponse.response()
                        .withStatusCode(HttpStatusCode.OK_200.code())
                        .withBody(getBodyByFileName("json/health.json"))
                        .withContentType(org.mockserver.model.MediaType.APPLICATION_JSON)
                );

        log.info(" > The {} path is mocked (mock-server).", HEALTH_PATH);
    }
